...
public class HandlePost extends HandleHttp {
  ...
  public boolean handlePost(HttpInfo httpInfo) throws IOException, ServletException, AgentAppServerException, SymbolException, RuntimeException, AgentServerException, JSONException, TokenizerException, ParserException {
    ...
    if (lcPath.equals("/agent_definitions")){
      String agentInstanceName = request.getParameter("name");
      String userName = request.getParameter("user");
      JSONObject agentDefinitionJson = getJsonRequest(httpInfo);

      if (agentInstanceName == null){
        throw new AgentAppServerBadRequestException("Missing agent class name query parameter");
      } else if (agentInstanceName.trim().length() == 0){
        throw new AgentAppServerBadRequestException("Empty agent class name path parameter");
      } else if (userName == null){
        throw new AgentAppServerBadRequestException("Missing user name query parameter ('user')");
      } else if (userName.trim().length() == 0){
        throw new AgentAppServerBadRequestException("Empty user name path parameter ('user')");
      } else if (agentDefinitionJson == null){
        throw new AgentAppServerBadRequestException("Invalid agent class JSON object");
      } else if (agentServer.agentDefinitions.containsKey(agentInstanceName)){
        throw new AgentAppServerBadRequestException("Agent class name already exists");
      } else if (! agentServer.users.containsKey(userName)){
        throw new AgentAppServerBadRequestException("User name does not exist: '" + userName + "'");
      } else if (! agentDefinitionJson.has("parameters")){
        throw new AgentAppServerBadRequestException("Agent class JSON is missing parameters list");
      } else {
        log.info("Creating new agent class named: " + agentInstanceName);


        String agentDefinitionName = null;
        if (agentDefinitionJson.has("description"))
          agentDefinitionName = agentDefinitionJson.optString("definition");
        AgentDefinition agentDefinition = agentServer.agentDefinitions.nameValueMap.get(userName).value.get(agentDefinitionName);
        ...
        if (agentDefinitionJson.has("parameter_values")){
          parameterValuesJson = agentDefinitionJson.optJSONObject("parameter_values");
          parameterValues = SymbolValues.fromJson(symbolTable, parameterValuesJson);
        }

        String description = null;
        if (agentDefinitionJson.has("description"))
          description = agentDefinitionJson.optString("description");

        long triggerInterval = agentDefinitionJson.optLong("trigger_interval", 0);
        long reportingInterval = agentDefinitionJson.optLong("reporting_interval", 0);

        boolean enabled = agentDefinitionJson.optBoolean("enabled", true);

        String created = agentDefinitionJson.optString("created");
        long timeCreated = -1;
        try {
        ...
        String modified = agentDefinitionJson.optString("modified");
        long timeModified = -1;
        try {
        ...
        if (parameterValues != null) {
          User user = agentServer.users.get(userName);
          AgentInstance agentInstance = new AgentInstance(user, agentDefinition, agentInstanceName, description, parameterValues, triggerInterval, reportingInterval, enabled, timeCreated, timeModified, state, false);
          log.info("Created new agent instance named: " + agentInstanceName);
          ...
          response.getWriter().println("Unable to agent class parse parameters");
          log.info("Unable to agent class parse parameters");
          ((Request)request).setHandled(true);
        }
      }
